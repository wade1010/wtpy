...
from strategies.TrendFlow import StraTrendFlow
...
if __name__ == "__main__":
    ...
    '''
    # 主力合约回测
    straInfo = StraTrendFlow(name='pydt_jm', code="DCE.jm.HOT", barCnt=50, period="d7", days=30, k1=0.1, k2=0.1, isForStk=False)
    ...
    if True:
        ...
        analyst = WtBtAnalyst()
        # 将回测的输出数据目录传递给绩效分析模块
        analyst.add_strategy("pydt_jm", folder="./outputs_bt/", init_capital=500000, rf=0.02, annual_trading_days=240)
        # 运行绩效模块
        analyst.run_new()
    else:
        # 使用pyfolio进行绩效分析
        analyze_with_pyfolio("./outputs_bt/pydt_jm/funds.csv")
